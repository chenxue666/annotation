#-*- encoding:utf-8 -*-
from __future__ import print_function
import pickle

import os.path
import io

from pathlib import Path
home = str(Path.home())

import sys
try:
    reload(sys)
    sys.setdefaultencoding('utf-8')
except:
    pass

import codecs
from textrank4zh import TextRank4Keyword, TextRank4Sentence

with open(home + '/coding/annotation/chinese_news.pkl', 'rb') as data_file:
    records_complete = pickle.load(data_file)

myfile = open('xyz_latest.txt', 'w')

for news in records_complete:
    title = news['title']
    content = news['content']
    time = news['updateTime']

    myfile.write("title：\n")
    myfile.write("{}\n".format(title))
    myfile.write("content：\n")
    myfile.write("{}\n".format(content))
    myfile.write("time：\n")
    myfile.write("{}\n".format(time))

    text = "。 ".join([title, content])
    # text = codecs.open('home + '/coding/annotation/try.txt', 'r', 'utf-8').read()
    tr4w = TextRank4Keyword()

    tr4w.analyze(text=text, lower=True, window=2)  # py2中text必须是utf8编码的str或者unicode对象，py3中必须是utf8编码的bytes或者str对象

    myfile.write("关键词：\n")
    # print( '关键词：' )
    for item in tr4w.get_keywords(20, word_min_len=1):
        # print(item.word, item.weight)
        myfile.write("{}, {}\n".format(item.word, str(item.weight)))


    # print()
    # print( '关键短语：' )
    myfile.write("\n关键短语：")
    for phrase in tr4w.get_keyphrases(keywords_num=20, min_occur_num= 2):
        # print(phrase)
        myfile.write("{}\n".format(phrase))

    tr4s = TextRank4Sentence()
    tr4s.analyze(text=text, lower=True, source = 'all_filters')

    # print()
    # print( '摘要：' )
    myfile.write("\n摘要：")
    for item in tr4s.get_key_sentences(num=3):
        # print(item.index, item.weight, item.sentence)  # index是语句在文本中位置，weight是权重
        myfile.write("{}, {}, {}\n".format(str(item.index), str(item.weight), item.sentence))

    # input("Press Enter to continue...")
